<?php
  ...
  $id = $_GET['id'];
  $sql = "SELECT trans_id, emp_id, client_name, cc_num, trans_date, amount as tamount FROM transaction WHERE trans_id = :id AND emp_id =:user_id ";
                     ...
                     <div class="col-md-6">
                       <select class="form-control js-example-basic-multiple" name="client[]" id='client' multiple="multiple">
                          <?php
                            ...
                            $sql = "SELECT * FROM clients WHERE concat(clients.client_name,' ', clients.client_branch) NOT IN (:customers) ORDER BY client_name asc, client_branch asc";
                            $stmt = $pdo -> prepare($sql);
                              ...
                              if ($count > 0) {
                               ...
                               while($row = $stmt -> fetch(PDO::FETCH_ASSOC)){
                                ...
                                $detail_id = trim($row['detail_id']);
                                  ...
                                  <td class='text-center'> <input type='checkbox' name='deletee[]' id='$i' class='try' value='$detail_id'> </td>
                                  </div>
                                  </tr>
                                  ";
                                }
         ...
         $.ajax({
                ...
                success:function(data){
                     if (data.status == 'success'){
                         ...
                         $('#cc_rep')[0].reset();
                         location.replace("petty-cash.php");
