...
$sql = "SELECT rem_id, rem_emp, rem_date, rem_ctr_no, rem_status FROM remittance_report WHERE rem_emp_id = :uid";
...
if( !empty($requestData['search']['value']) ) {
  ...
  $sql.= "WHERE rem_ctr_no LIKE '".$requestData['search']['value']."%' ";  
  $sql.= "AND rem_emp_id = :uid ";  
  $stmt = $pdo -> prepare($sql);
  $stmt -> bindParam(":uid", $user_id);
  $stmt -> execute();  
  ...
  $sql = "SELECT rem_id, rem_emp, rem_date, rem_ctr_no, rem_status
          FROM remittance_report WHERE rem_emp_id = :uid ";
  $sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]." ".$requestData['order'][0]['dir']." LIMIT ".$requestData['start']." ,".$requestData['length']."   "; 
  $stmt = $pdo -> prepare($sql);
  $stmt -> bindParam(":uid", $user_id);
  $stmt -> execute();
